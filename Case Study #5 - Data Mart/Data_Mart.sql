...
FROM sales_cte
GROUP BY year_calendar,  month_number
ORDER BY year_calendar, month_number;


-- Tương tự cách 2 (có điều chỉnh 1 chút)
with sale as
(
select 
	year_calendar,
...
select 
	year_calendar, month_number, max(total_sales) total_sales,
	cast(100.0*sum(case when platform = 'Shopify' then sales end)/max(total_sales) as decimal(5,2)) pct_shopify,
	cast(100.0*sum(case when platform = 'Retail' then sales end)/max(total_sales) as decimal(5,2)) pct_retail
from sale
group by year_calendar, month_number
order by year_calendar desc, month_number desc
